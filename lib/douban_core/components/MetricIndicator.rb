module WCF
  module Components
    ...
    # Represents metric indicator in the application usually by the title.
    class MetricIndicator < Metric
      ...
      def initialize(title = nil, under_locator = nil, index = 1)
        if title == "[unnamed]" or title.to_s == ''
          super("#{under_locator}//div[contains(@class, 'componentBody') and div/div[contains(@style,'metric-indicator')]][#{index}]")
        else
          super("#{under_locator}//div[contains(@class, 'componentBody') and div/div[contains(@style,'metric-indicator')] and div[1][contains(text(), '#{title}')]][#{index}]")
        end
      end
      ...
      def severe
        wait
        icon = Element.new(@locator + "/div/div[contains(@style,'metric-indicator')]")
